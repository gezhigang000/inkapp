...
|------|---------------|-----------------|
| LLM 调用 | 1 次请求 → 1 次响应 | 模板定义轮次，LLM 自主决定何时结束 |
...
```
┌─────────────────────────────────────────┐
│  系统提示词 (agent_prompts.py)           │  ← 只有工具说明 + 基础 HTML 规则
├─────────────────────────────────────────┤
│  模板 prompt (prompt-templates.ts)       │  ← 主导行为：工作流、质量标准、排版
├─────────────────────────────────────────┤
│  文件格式规则 (条件追加)                  │  ← 仅当有上传文件时追加
└─────────────────────────────────────────┘
```

- 系统层不叠加工作流程和质量标准，避免与模板 prompt 冲突
- 模板 prompt 是用户可编辑的，所见即所得
- 无模板时才使用默认的"深度调研"工作流
- 所有模板内置 HTML 质量规则（中英文间距、表格对比度等），由 prompt 层强制约束

### 排版样式（v1.9.0+）

每个模板可指定 `layoutStyle`，前端→sidecar→LLM 全链路传递，由 `agent_prompts.py` 注入对应排版指令：

| layoutStyle | 名称 | 说明 |
|-------------|------|------|
| `modular` | 模块式 | PART 1 / PART 2 分区，不同主题色 |
| `chapter` | 章节式 | 一、二、三 或 01 / 02 / 03 编号 |
| `card` | 卡片式 | 独立卡片，无编号，视觉分隔 |
| `narrative` | 叙事式 | 自然段落流，无分区标签 |
| `custom` | 自定义 | 由提示词自行控制排版结构 |

### 轮次控制

每个 Agent 模板内置 `maxTurns` 和 `layoutStyle`，前端透传给 sidecar，用户不可见：

| 模板 | agentMode | maxTurns | layoutStyle | 原因 |
|------|-----------|----------|-------------|------|
| 深度研究 | ✅ | 15 | modular | 多轮搜索 + 深度分析 |
| 数据分析 | ✅ | 10 | chapter | run_python 分析 + 图表生成 |
